...
const legacyLockMarker = "# Generated by tripgoctl. Do not edit manually.\n"
...
func Write(target string, bundle Bundle) error {
	...
	if err := removeLegacyLock(target); err != nil {
		return err
	}
	return nil
}

func removeLegacyLock(target string) error {
	path := filepath.Join(target, "environment.lock")
	info, err := os.Lstat(path)
	if errors.Is(err, os.ErrNotExist) {
		return nil
	}
	if err != nil {
		return fmt.Errorf("generator: inspect legacy environment.lock: %w", err)
	}
	if info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() {
		return nil
	}
	content, err := os.ReadFile(path) // #nosec G304 -- path is constrained beneath the explicit output directory.
	if err != nil {
		return fmt.Errorf("generator: read legacy environment.lock: %w", err)
	}
	if !bytes.HasPrefix(content, []byte(legacyLockMarker)) {
		return nil
	}
	if err := os.Remove(path); err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("generator: remove legacy environment.lock: %w", err)
	}
	return nil
}
