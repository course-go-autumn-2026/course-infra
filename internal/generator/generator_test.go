...
func TestCustomEnvDoesNotLeakIntoManifestOrLock(t *testing.T) {
	t.Parallel()
	...
	if !reflect.DeepEqual(baseBundle, customBundle) {
		t.Fatal("custom env changed manifests or lock metadata")
	}
	if bytes.Contains(customBundle.Lock, []byte("do-not-store")) {
		t.Fatal("custom env value leaked into lock")
	}
}
...
func TestWriteRefusesForeignDirectory(t *testing.T) {
	t.Parallel()
	...
	if err := os.WriteFile(filepath.Join(target, "notes"), []byte("mine"), 0o600); err != nil {
		t.Fatal(err)
	}
	err := generator.Write(target, generator.Bundle{})
	if err == nil || !strings.Contains(err.Error(), "not owned") {
		t.Fatalf("Write() error = %v", err)
	}
}

func TestWriteRefusesSymlinkedLock(t *testing.T) {
	t.Parallel()
	...
	if err := os.WriteFile(lock, bundle.Lock, 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink(lock, filepath.Join(target, "environment.lock")); err != nil {
		t.Fatal(err)
	}
	if err := generator.Write(target, bundle); err == nil || !strings.Contains(err.Error(), "not owned") {
		t.Fatalf("Write() error = %v", err)
	}
}
...
func writeGolden(target string, bundle generator.Bundle) error {
	...
	return os.WriteFile(filepath.Join(target, "environment.lock"), bundle.Lock, 0o600)
}
...
func assertGolden(t *testing.T, target string, bundle generator.Bundle) {
	...
	assertFile(t, filepath.Join(target, "environment.lock"), bundle.Lock)
...
func assertKubernetesDocuments(t *testing.T, bundle generator.Bundle, config environment.Config) {
	...
	for _, file := range bundle.Files {
		...
		if file.SHA256 != digest || !bytes.Contains(bundle.Lock, []byte(digest)) {
			t.Errorf("hash for %s is not represented in lock", file.Name)
		}
	}
}
