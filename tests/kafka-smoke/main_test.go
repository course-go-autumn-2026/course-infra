...
func TestRecordMatchesOptionalExpectedKey(t *testing.T) {
	t.Parallel()
	record := &kgo.Record{Key: []byte("stage9-lab4"), Value: []byte("only-lab4")}
	for _, test := range []struct {
		...
		{name: "matching key and value", message: "only-lab4", expectedKey: "stage9-lab4", want: true},
		{name: "wrong key", message: "only-lab4", expectedKey: "another", want: false},
		{name: "wrong value", message: "other", expectedKey: "stage9-lab4", want: false},
